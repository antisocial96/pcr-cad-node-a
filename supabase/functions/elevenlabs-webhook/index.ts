...
const constructWebhookEvent = async (req: Request, secret: string) => {
  ...
  try {
    ...
    const signatureHex = "v0=" + Array.from(new Uint8Array(signatureBytes))
  ...
  if (event?.type === "post_call_transcription") {
    ...
    try {
      ...
      if (existingCall) {
        ...
        return new Response(
          JSON.stringify({ received: true, updated_call: updated[0] }),
        ...
        return new Response(
          JSON.stringify({ received: true, created_call: created[0] }),
          { status: 200, headers: corsHeaders }
        );
      }
    } catch (dbErr) {
